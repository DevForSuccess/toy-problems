...
Tree.prototype.DFSelect = function (filter) {
    ...
    let _dfSelect = function (tree, depth) {
        if (filter(tree.value, depth)) {
            result.push(tree.value);
        }
        tree.children.forEach(child => _dfSelect(child, depth + 1));
    }
    _dfSelect(this, 0);
    return result;
};
...
let test = root1.DFSelect(function (value, depth) {
    return value % 2;
})
// [1, 5, 3, 7]
console.log(test);

test = root1.DFSelect(function (value, depth) {
    return depth === 1;
})
// [2, 3]
console.log(test);